...
public:
	...
	void assign(size_type count, const T& value)
	{
		...
		else
		{
			...
			pointer new_begin = detail::allocate<T>(new_cap);
	...
	iterator insert(const_iterator pos, size_type count, const T& value)
	{
		...
		if (old_size + count <= capacity())
		{
			...
			if (count > elems_after)
			{
				...
				detail::destroy_guard<T> dguard1(_end, _end + (count - elems_after));

				std::uninitialized_move_n(insert_pos, elems_after, insert_pos + count);
				detail::destroy_guard<T> dguard2(insert_pos + count, insert_pos + count + elems_after);
			...
			else
			{
				...
				detail::destroy_guard<T> dguard(_end, _end + count);
		...
		else
		{
			...
			detail::destroy_guard<T> dguard1(insert_pos, insert_pos + count);

			relocate_n(_begin, offset, new_begin);
			detail::destroy_guard<T> dguard2(new_begin, new_begin + offset);
	...
	iterator emplace(const_iterator pos, Args&&... args)
	{
		...
		if (_end != _end_cap)
		{
			...
			else
			{
				...
				++_end;
				detail::destroy_guard<T> dguard(_end - 1, _end);
	...
	void resize(size_type count)
	{
		...
		else if (count > old_size)
		{
			...
			else
			{
				...
				detail::destroy_guard<T> dguard(new_begin + old_size, new_begin + count);
...
private:
	...
	void construct_n(size_type count, Args&&... args)
	{
		...
		pointer new_begin = detail::allocate<T>(count);
	...
	void change_array(pointer new_begin, size_type new_size, size_type new_cap)
	{
		...
		{
			std::destroy(_begin, _end);
			detail::deallocate(_begin, capacity());
		}
