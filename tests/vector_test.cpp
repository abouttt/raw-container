...
// Helper: compare vector with initializer list
template <typename T>
...
TEST(VectorTest, Comparisons)
{
	...
	raw::vector<int> v3 = { 1, 2, 4 };
	raw::vector<int> v4 = { 1, 2 };
...
TEST(VectorTest, SelfAssignmentAssign)
{
	// Self-referential assign (range)
	raw::vector<int> v = { 1, 2, 3, 4, 5 };
	v.assign(v.begin(), v.end());
	EXPECT_TRUE(equal(v, { 1, 2, 3, 4, 5 }));
	...
	// Self-referential assign (single value)
	raw::vector<int> w = { 1, 2, 3 };
	w.assign(3, w[0]);
	EXPECT_TRUE(equal(w, { 1, 1, 1 }));
}
